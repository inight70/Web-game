// ==========================================
// محرك اللعبة (Game Engine) - الإصدار الجذري والآمن
// ==========================================
...
window.gameEngine = {
    ...
    startGameWithBots: async function() {
        ...
        try {
            ...
            // إضافة البوتات لو كان العدد أقل من 6
...
// ==========================================
// الرادار العالمي (Global Radar) - يحل مشكلة الجهاز الثاني نهائياً
// ==========================================
...
window._masterGameRadar = setInterval(() => {
    ...
    // إذا الغرفة بدأت اللعب، وأنت لست في صفحة اللعبة، انقلك فوراً!
    if(window.currentRoomData.status === 'playing' && currentPath !== 'game') {
        console.log("🚀 Radar Triggered! Moving to Game Page.");
        if(window.loadFragment) window.loadFragment('game', null);
    }
    
    // إذا اللعبة انتهت وعدنا للانتظار، ارجعك للوبي!
